...
function buildPrompt(project: string, runId: string, workflowIds: string[], caseIds: string[]): string {
  ...
  return [
    ...
    "(c) Registration is SUCCESS only when at least one explicit success signal is observed: (1) navigated to OTP/verification step, OR (2) clear success toast/message, OR (3) localStorage 'verifications' exists with a non-empty meaningful value. If URL stays on signup/register without explicit success, treat as FAIL.",
